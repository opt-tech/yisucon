...
const (
	sessionName     = "optwitter_session"
	sessionSecret   = "optwitter"
	perPage         = 50
	optomoEndpoint = "http://localhost:8081"
)
...
func loadFriends(name string) ([]string, error) {
	resp, err := http.DefaultClient.Get(optomoEndpoint + pathURIEscape("/"+name))
	if err != nil {
		return nil, err
	}
...
func initializeHandler(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := http.Get(fmt.Sprintf("%s/initialize", optomoEndpoint))
	if err != nil {
		badRequest(w)
		return
	}
...
func followHandler(w http.ResponseWriter, r *http.Request) {
	...
	req, err := http.NewRequest(http.MethodPost, optomoEndpoint+pathURIEscape("/"+userName), bytes.NewBuffer([]byte(jsonStr)))
...
func unfollowHandler(w http.ResponseWriter, r *http.Request) {
	...
	jsonStr := `{"user":"` + r.FormValue("user") + `"}`
	req, err := http.NewRequest(http.MethodDelete, optomoEndpoint+pathURIEscape("/"+userName), bytes.NewBuffer([]byte(jsonStr)))
...
func main() {
	host := os.Getenv("OPTWITTER_DB_HOST")
	if host == "" {
		host = "localhost"
	}
	port := os.Getenv("OPTWITTER_DB_PORT")
	if port == "" {
		port = "3306"
	}
	user := os.Getenv("OPTWITTER_DB_USER")
	if user == "" {
		user = "root"
	}
	password := os.Getenv("OPTWITTER_DB_PASSWORD")
	dbname := os.Getenv("OPTWITTER_DB_NAME")
	if dbname == "" {
		dbname = "optwitter"
	}
